...
$(document).ready(function(){
  ...
  if ($('.wrap').hasClass('configureDetail')){
    ...
    if (type == 'focus'){
      $('.configure-list .tip').html('鈥淪鈥� - 鏍囧噯閰嶇疆锛屸€�-鈥� - 鏃犳閰嶇疆锛屸€淥鈥� - 閫夎閰嶇疆锛�<br>鈥淥1鈥� - 鏅鸿兘瀹夊叏閫夎鍖匒锛屸€淥2鈥� - 鏅鸿兘瀹夊叏閫夎鍖匓銆�');
    }
    ...
    $(document).delegate('.go-detail', 'click', function(){
      ...
      if ($('.car-image').hasClass('close')){
        if (ford.isAndroid){
          ...
          divTop = divTop - $('.car-image').height() - $('.configure-list').height() - 50 +49;  //50 padding-top  49 calc(100% - 49px)涓嶈绠�49
        } 
      } else {
        if (ford.isAndroid){
          divTop = divTop - ($('.car-image').height()-60) - 95 - 50 +49; //60 鎶樺彔鍚庨珮搴�
        } else {
          divTop = divTop - ($('.car-image').height()-60) - $('.configure-list').height() - 50 +49; //60 鎶樺彔鍚庨珮搴�
        }
      }
      $('.content').scrollTo({
        durTime: 100,
    ...
    $('.content').on('scroll', function() {
      ...
      // 10 鎻愬墠10px 瑙﹀彂
      //  parseInt($('#detail'+i).height()  detail鍖哄潡楂樺害
...
function isSupportSticky() {//鍒ゆ柇鏄惁鏀寔sticky
...
//箭头位置
$(function(){
	var arrowHeight=$(".item a img").height()/2-20;
	console.log(arrowHeight)
	$(".carousel-control span.left-carsour").css({"top":arrowHeight})
	$(".carousel-control span.right-carsour").css({"top":arrowHeight})
})
